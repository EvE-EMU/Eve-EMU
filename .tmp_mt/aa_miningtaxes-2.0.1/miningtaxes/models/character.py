...
WORMHOLE_SYSTEM_RE = re.compile(r"^J([0-9]+)$")


def _rounded_security_status(system: SolarSystem) -> Optional[float]:
    if system.security_status is None:
        return None
    return round(system.security_status, 1)


def _is_trig_space(system: SolarSystem) -> bool:
    return system.visual_effect == "TRIGLAVIAN_HOME"


def _is_w_space(system: SolarSystem) -> bool:
    if _is_trig_space(system):
        return False
    return bool(system.name and WORMHOLE_SYSTEM_RE.match(system.name))


def _is_high_sec(system: SolarSystem) -> bool:
    sec = _rounded_security_status(system)
    return sec is not None and 0.5 <= sec <= 1.0


def _is_low_sec(system: SolarSystem) -> bool:
    sec = _rounded_security_status(system)
    return sec is not None and 0.1 <= sec <= 0.4


def _is_null_sec(system: SolarSystem) -> bool:
    if _is_w_space(system) or _is_trig_space(system):
        return False
    sec = _rounded_security_status(system)
    return sec is not None and -1.0 <= sec <= 0.0
...
class CharacterQuerySet(models.QuerySet):
    ...
    def user_has_access(self, user: User) -> models.QuerySet:
        """Return all characters a user is allowed to view."""
        if user.has_perm("miningtaxes.auditor_access"):
            return self.all()
...
class Character(CharacterAbstract):
    ...
    def update_mining_ledger(self, token: Token):
        ...
        for entry in entries:
            eve_type = ItemType.objects.get(id=_esi_value(entry, "type_id"))
            if (
                eve_type.group_id in PriceGroups.moon_ore_groups
            ) and MININGTAXES_TAX_ONLY_CORP_MOONS:
                continue

            eve_solar_system = SolarSystem.objects.get(
                id=_esi_value(entry, "solar_system_id")
            )
            try:
                ...
                if row.quantity != _esi_value(entry, "quantity"):
                    row.quantity = _esi_value(entry, "quantity")
                    row.save()
                    row.calc_prices()
                ...
                row = self.mining_ledger.create(
                    ...
                    eve_solar_system=eve_solar_system,
                    eve_type=eve_type,
                    quantity=_esi_value(entry, "quantity"),
                )
                row.calc_prices()
    ...
    def json_standardize(self, months):
        newmonths = {}
        for h in months:
            if type(h["month"]) is dt.datetime:
...
class CharacterMiningLedgerEntry(models.Model):
    ...
    def calc_prices(self, corpmoon=False):
        ...
        if corpmoon is False and (
            ...
            or (
                ...
                and (
                    ...
                    or (
                        MININGTAXES_TAX_HISEC is False
                        and _is_high_sec(self.eve_solar_system)
                    )
                    or (
                        MININGTAXES_TAX_LOSEC is False
                        and _is_low_sec(self.eve_solar_system)
                    )
                    or (
                        MININGTAXES_TAX_NULLSEC is False
                        and _is_null_sec(self.eve_solar_system)
                    )
                    or (
                        MININGTAXES_TAX_JSPACE is False
                        and _is_w_space(self.eve_solar_system)
                    )
                    or (
                        MININGTAXES_TAX_POCHVEN is False
                        and _is_trig_space(self.eve_solar_system)
                    )
                )
            )
        ):
