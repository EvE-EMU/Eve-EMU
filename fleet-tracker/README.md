...
| Ship while in fleet | **`GET /characters/{character_id}/ship/`** — scope **`esi-location.read_ship_type.v1`**. |
...
- **Participant OAuth**: each user links a character; store refresh token encrypted at rest; scopes at minimum:  
  `esi-fleets.read_fleet.v1` `esi-location.read_location.v1` `esi-location.read_ship_type.v1` `esi-killmails.read_killmails.v1`  
