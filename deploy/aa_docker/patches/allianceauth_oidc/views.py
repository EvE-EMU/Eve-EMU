...
def check_user_state_and_groups(user: User, app):
    ...
    if _states:
        ...
        if user_state is not None:
            state_access = app.states.filter(pk=user_state.pk).exists()
        log.debug(
            f"OAUTH STATE User: {user_state} APP: {app.states.all()}")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if status == 200:
            try:
                ...
                log.warning("OAUTH token exchange denied for %s", request.user)
                body = json.dumps({
                    "error": "access_denied",
