...
def run_price_config(id: int) -> None:
    ...
    orders_location_type = orders_location.filter(
        Q(item_type__in=config.item_type.all()) | Q(item_type__group__in=config.item_group.all())
        | Q(item_type__market_group__in=config.item_market_group.all())
        | Q(item_type__market_group__parent_group__in=config.item_market_group.all())
        | Q(item_type__market_group__parent_group__parent_group__in=config.item_market_group.all())
        | Q(item_type__market_group__parent_group__parent_group__parent_group__in=config.item_market_group.all())
        | Q(item_type__market_group__parent_group__parent_group__parent_group__parent_group__in=config.item_market_group.all())
    )
...
def run_margin_config(id: int) -> None:
    ...
    item_types = config.item_type.all() | ItemType.objects.filter(group__in=config.item_group.all()) \
        | ItemType.objects.filter(market_group__in=config.item_market_group.all()) \
        | ItemType.objects.filter(market_group__parent_group__in=config.item_market_group.all()) \
        | ItemType.objects.filter(market_group__parent_group__parent_group__in=config.item_market_group.all()) \
        | ItemType.objects.filter(market_group__parent_group__parent_group__parent_group__in=config.item_market_group.all()) \
        | ItemType.objects.filter(market_group__parent_group__parent_group__parent_group__parent_group__in=config.item_market_group.all())
...
def update_all_type_statistics() -> None:
    ...
    for item_type in ItemType.objects.filter(market_group__isnull=False, published=1):
